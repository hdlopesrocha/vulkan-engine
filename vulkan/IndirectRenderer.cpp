...
void IndirectRenderer::cleanup(VulkanApp* app) {
    ...
    if (compactIndirectBuffer.buffer != VK_NULL_HANDLE) vkDestroyBuffer(app->getDevice(), compactIndirectBuffer.buffer, nullptr);
    if (compactIndirectBuffer.memory != VK_NULL_HANDLE) vkFreeMemory(app->getDevice(), compactIndirectBuffer.memory, nullptr);
    if (modelsBuffer.buffer != VK_NULL_HANDLE) vkDestroyBuffer(app->getDevice(), modelsBuffer.buffer, nullptr);
    if (modelsBuffer.memory != VK_NULL_HANDLE) vkFreeMemory(app->getDevice(), modelsBuffer.memory, nullptr);
    if (boundsBuffer.buffer != VK_NULL_HANDLE) vkDestroyBuffer(app->getDevice(), boundsBuffer.buffer, nullptr);
    if (boundsBuffer.memory != VK_NULL_HANDLE) vkFreeMemory(app->getDevice(), boundsBuffer.memory, nullptr);
    if (compactIndirectBuffer.buffer != VK_NULL_HANDLE) vkDestroyBuffer(app->getDevice(), compactIndirectBuffer.buffer, nullptr);
    if (compactIndirectBuffer.memory != VK_NULL_HANDLE) vkFreeMemory(app->getDevice(), compactIndirectBuffer.memory, nullptr);
    if (visibleCountBuffer.buffer != VK_NULL_HANDLE) vkDestroyBuffer(app->getDevice(), visibleCountBuffer.buffer, nullptr);
    if (visibleCountBuffer.memory != VK_NULL_HANDLE) vkFreeMemory(app->getDevice(), visibleCountBuffer.memory, nullptr);
...
uint32_t IndirectRenderer::addMesh(VulkanApp* app, const Geometry& mesh, const glm::mat4& model) {
    ...
    // Rebuild GPU buffers to include new mesh (may be optimized later)
    rebuild(app);
...
void IndirectRenderer::drawMergedWithCull(VkCommandBuffer cmd, const glm::mat4& viewProj, VulkanApp* app, uint32_t maxDraws) {
    ...
    // If compute pipeline not available, fall back to drawing all indirect commands
    if (computePipeline == VK_NULL_HANDLE || compactIndirectBuffer.buffer == VK_NULL_HANDLE) {
        VkDeviceSize offsets = 0;
        VkBuffer vbs[] = { vertexBuffer.buffer };
        VkDeviceSize vOffsets[] = { 0 };
        vkCmdBindVertexBuffers(cmd, 0, 1, vbs, vOffsets);
        vkCmdBindIndexBuffer(cmd, indexBuffer.buffer, 0, VK_INDEX_TYPE_UINT32);
        if (indirectBuffer.buffer != VK_NULL_HANDLE && indirectCommands.size() > 0) {
            vkCmdDrawIndexedIndirect(cmd, indirectBuffer.buffer, 0, static_cast<uint32_t>(indirectCommands.size()), sizeof(VkDrawIndexedIndirectCommand));
        }
        return;
    }
    ...
    barriers[1].dstAccessMask = VK_ACCESS_INDIRECT_COMMAND_READ_BIT | VK_ACCESS_TRANSFER_READ_BIT | VK_ACCESS_SHADER_READ_BIT;
    ...
    // Issue indirect-count draw if supported, otherwise draw up to capacity
    ...
    if (cmdDrawIndexedIndirectCount) {
        ...
        // Fallback: issue indirect draw for full capacity (visible commands will be at start)
...
void IndirectRenderer::updateModelsDescriptorSet(VulkanApp* app, VkDescriptorSet ds) const {
    // Allocate and update a dedicated material-style descriptor set for the Models SSBO.
    // Writing into an externally-provided descriptor set can be unsafe if the layout
    // differs from the material descriptor set layout; allocate our own set instead.
    if (modelsBuffer.buffer == VK_NULL_HANDLE) return; // nothing to bind yet

    VkDescriptorSet target = app->createMaterialDescriptorSet();
    ...
    // Register the allocated descriptor set so the app can track it (debugging/widgets)
