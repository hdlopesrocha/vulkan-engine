...
void PostProcessRenderer::createPipeline(VulkanApp* app) {
    ...
    pipelineInfo.renderPass = app->getSwapchainRenderPass();
    pipelineInfo.subpass = 0;

    if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &pipeline) != VK_SUCCESS) {
        std::cerr << "[PostProcessRenderer] Warning: Failed to create post-process pipeline" << std::endl;
        pipeline = VK_NULL_HANDLE;
    }
    if (pipeline != VK_NULL_HANDLE) {
        app->resources.addPipeline(pipeline, "PostProcessRenderer: pipeline");
        std::cout << "[PostProcessRenderer] Created post-process pipeline" << std::endl;
    }
