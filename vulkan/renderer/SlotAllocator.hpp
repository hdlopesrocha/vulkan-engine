...
// Manages a pre-allocated pool of "slots" (draw entries) within the
// indirect command list. A slot is a single draw entry that identifies ONE
// chunk; the chunk's vertex/index data lives in the shared element pools
// managed by PackedSpaceAllocator. The allocator never compacts: once a slot
// index is assigned, it remains valid until explicitly freed. Freed slots
// are recycled to a free list.
//
