...
layout(location = 3) in vec4 tc_fragTangent[]; // now includes handedness in .w
...
layout(location = 9) in vec4 tc_fragLocalTangent[];
...
layout(location = 3) out vec4 fragTangent; // world-space tangent + handedness
...
void main() {
    ...
    vec4 localTangent4 = tc_fragLocalTangent[0] * bc.x + tc_fragLocalTangent[1] * bc.y + tc_fragLocalTangent[2] * bc.z;
    vec3 localTangent = localTangent4.xyz;
    ...
    fragTangent = vec4(normalize(mat3(ubo.model) * localTangent), localTangent4.w);
