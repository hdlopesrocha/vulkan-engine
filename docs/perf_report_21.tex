...
\\ \textbf{Implemented as a per-pixel identity early-out, not a UBO bit:}
baked non-identity vertex tints exist (\texttt{Simple/NormalBrush::paintHSV},
baked at \texttt{space/Octree.cpp:1210}), so a uniform gate would need global
tracking of resident tint state with staleness hazards. The per-pixel compare
is uniform wherever the tint is uniform (everywhere except tint boundaries),
always correct with no CPU plumbing, and applies in \texttt{BRUSH\_PASS} too;
inexact interpolation safely falls into the conversion path. Same fix applied
to the identical pattern in \texttt{water\_surface.glsl:1659--1665} (H5 was
solid-only; the water instantiation matched). Static size grows $\sim$100
words (branch around kept code) --- the win is dynamic: $\sim$5 ALU compare
vs $\sim$60 ALU divergent round-trip per pixel. Side effect: identity pixels
no longer pay float round-trip error ($\sim$1e-7 diff vs before, an
improvement; set A/B thresholds accordingly).
\end{tcolorbox}
...
\texttt{shaders/includes/rt\_reflection.glsl}. Validation: reflection
screenshots identical, static ALU reduction in the RT fragment variants,
validation-clean.
\\ \textbf{Implemented with two corrections:} (1) \emph{Exactness by
construction.} A first cut skipped the cascade-0 test on hinted hits, which
is wrong where cascade boxes overlap: the full path always tests cascade 0
first, so the shipped fast path reproduces that test (one matvec, as today)
and only past it projects the hinted cascade. Strict-inside then provably
equals the blend-1.0 early-out (same coordinates, same fetch); all other
cases fall back to the single full-path call with the projection reused.
No visual change is possible, by proof rather than by testing. (2)
\emph{Honest sizing.} The win lands only on strict cascade-1/2 secondaries
(up to one matvec + blend ALU + blend-zone fetch each); cascade-0 and edge
hits cost exactly today's path, wrong-hint edge hits pay one extra
projection. Static size \emph{grows} ($\sim$+5\% on the water RT module from
inliner-duplicated fallback bodies) --- the win is dynamic, and the report's
``up to three projections'' was the worst case, not the common one. Water
call sites keep the old signatures via overload wrappers (bit-identical);
only the solid chain (free primary hint) and the bounce recursion (hint
forwarded) convert.
\end{tcolorbox}
...
\\ \textbf{Implemented with narrowed scope:} the interpolant is eliminated
everywhere (FS declarations plus TCS/TES passthrough removed; disassembly
confirms location 22 absent from all solid stage interfaces). Fetch-side and
stride changes are partial \emph{by design}: the shared tess VS keeps reading
\texttt{inColor} (filtering its $\sim$15 pipeline attribute lists plus the
water tess family was rejected as risk without runtime validation), while the
two owned no-tess twins (depth, shadow) got filtered attributes (disassembly
confirms no location-1 input) for the 12\,B fetch saving where the VS is a
plain streamer. The 64 $\rightarrow$ 48 stride repack is deferred: the RT
shaders hardcode the 16-float vertex layout (\texttt{kVertStride}, normal at
floats 8--10, brush index at 11 --- 44 \texttt{rtSceneVerts} references across
\texttt{rt\_reflection.glsl}, \texttt{rt\_scene\_sample.glsl},
\texttt{solid\_surface.glsl}, \texttt{water\_surface.glsl}), and a missed
offset corrupts reflections silently, undetectable without executing the app.
That repack requires a runnable validation pass.
\end{tcolorbox}
...
\\ \textbf{Implemented differently:} the naive guard breaks the debug
varying --- \texttt{tc\_fragTessLevel} is written per-invocation from the
edge values, so invocations 1--2 would emit garbage. The textbook fix
(compute once, broadcast via \texttt{shared} + \texttt{barrier()}) does not
compile: Vulkan GLSL has no cross-invocation communication in TCS
(\texttt{'shared' : not supported in this stage}). The shipped design moves
the heat computation to the TES, which reads the TCS levels as patch inputs
(\texttt{gl\_TessLevelOuter/Inner} carry the exact TCS-written values): same
max chain, same value, evaluated per TES vertex instead of redundantly per
TCS invocation, and the TCS debug output is deleted. Writing the levels from
invocation 0 only is additionally \emph{more} correct than the status quo
(no benign 3-way store race). Heatmap identical by construction; confirm in
\texttt{DEBUG\_MODE\_TESS\_HEAT}.
\end{tcolorbox}
...
drivers).
\\ \textbf{Implemented, extended to the whole file:} the same
implicit-LOD-in-divergent-flow pattern appeared in four more spots of the
same shader (\texttt{vegColorTex}, \texttt{brushDepthTex},
\texttt{sdfColorTex}, \texttt{bboxColorTex} --- all fetched under
per-pixel-varying conditions, all single-level offscreen targets where LOD 0
is the only correct level). All converted; the remaining implicit taps are
top-level uniform flow and stay. No visual change is expected except at
silhouette boundaries, where the old derivatives were undefined.
\end{tcolorbox}
...
validation-clean.
\\ \textbf{Implemented as specified:} \texttt{applyMinimal} sets $0.5$,
\texttt{applyMaximum} sets $1.0$. No new plumbing was needed --- the
per-frame \texttt{waterRenderScale} check (\texttt{MyApp.cpp:1182--1184})
already recreates all water targets (color, body, column, geom depth,
back-face) under a device idle, and the composite was designed for
mismatched sizes (closest-of-four depth taps). The blur disc is authored in
screen-space units (\texttt{water\_surface.glsl:1773--1774}), so the look
survives the downscale; only the body texel density halves.
\end{tcolorbox}
...
validation-clean.
\\ \textbf{Implemented better than specified:} the buffer sizing in step (1)
was a red herring --- \texttt{numCmds} was already
\texttt{max(meshCount, capacity) = capacity}, and buffer \emph{size} costs no
frame time anyway; the cost is the dispatch, which always covers the fixed
slot capacity. The real defect was the unconditional zero-fills of the main
compact/count buffers inside the cascade-only cull path
(\texttt{IndirectRenderer.cpp}), which destroyed the cull-task results the
async task needed. But the cascade dispatch never writes the main streams
(\texttt{pc.doMain==0} gates them in \texttt{indirect.comp}, verified;
cascade atomics target the cascade count buffers), so the fills were
vestigial destruction. They are now skipped when \texttt{doMain==false}
(mirroring the adjacent \texttt{visibleLods} guard), the entire task-local
path is deleted ($\sim$130 lines: dispatch, buffers, descriptor ring,
pre-allocation), and the back-face pass reuses the main output unconditionally
under the existing tlSolid-transitive ordering (which already covers the
shadow restore). Follow-up, not done: the shadow-blur \emph{restore}
re-dispatches are now redundant identically-valued rewrites (kept for their
late-upload meta flush); removing them needs that flush moved to pass
prologues.
\end{tcolorbox}
...
\\ \textbf{Declined the headline, shipped the sound remainder:} per-chunk
constants would staircase the depth signal across chunks, and the audit
showed the signal feeds the always-live tint path
(\texttt{regionDepth} at \texttt{water\_surface.glsl:1558}) --- flat tint per
chunk is a visible regression by mechanism, unvalidatable without executing.
A backface-validity flag was also declined (one fetch saved, only in flat
mode, for cross-file UBO plumbing with transition-frame staleness). What
shipped is exact: \texttt{waterShoreDirFromSamples} takes the
caller-reconstructed center (\texttt{solidWorldH}/\texttt{backWorldH},
hoisted to call scope) instead of rebuilding it, saving one inverse-VP
matvec per call on \emph{both} geometry paths (4 call sites, bit-identical
by construction --- same expression, same inputs). The remaining 6 fetches
and reconstructs are load-bearing (tint needs spatial variation;
\texttt{max()} needs both surfaces). Module 4038 $\rightarrow$ 4009 words;
the win is dynamic ($\sim$2 matvecs per wavy vertex).
\end{tcolorbox}
...
trivial fix. \\ \textbf{Shipped:} uniform \texttt{e > 1.0} fast path around
the \texttt{pow} (exact for \texttt{wt >= 0}); the default exponent is $1.0$,
so default configs skip it entirely with no divergence.
...
\\ \textbf{Shipped as prescribed} (uniform branch): the check never fires
on valid assets, and a regression would still read as black in the normal
view --- equally visible. Retained in every debug view, where diagnosis
happens.
...
\\ \textbf{Shipped:} body/column allocate only while
\texttt{geometryBodyAttachmentsActive()} (exact pass requirement, including
the missing-module fallback), with a \texttt{waterBodyTargetsStale()}
predicate driving regrow/shrink on the existing per-frame idle recreate
path. Begin/end/clear NULL-guard; the composite binds the 1x1 dummy when
unallocated. No mid-frame allocation, no new synchronization.
...
H5 & HSV gate: per-pixel identity skip (shipped, +water) & $-60$ ALU/px at uniform tint \\
H6 & Cascade reuse in secondary hits (shipped, exact) & $\le$1 matvec + blend per c1/c2-strict hit \\
H7 & Dead color: interpolant out, twins filtered (stride stays 64) & $-1$ varying, $-12$\,B/vtx on twins \\
H8 & TCS invocation guard & $-67$\% solid TCS ALU \\
M9--M12 & Composite LOD, scale, cull, VS VTF & additive, each 2--10\% \\
L13--L15 & Hygiene, all shipped & pow skip, NaN gate, lazy body/column \\
\bottomrule
\end{tabular}
\end{table}
...
% =====================================================================
\section*{Appendix: Implementation Status}
% =====================================================================
\addcontentsline{toc}{section}{Appendix: Implementation Status}

Written at audit time, the report above describes the pre-fix tree. Disposition
of each item (commit hashes on \texttt{master}); correction notes live inline
...
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Item} & \textbf{Status} & \textbf{Commit / note} \\
\midrule
C1 & Partial (depth+shadow twins) & \texttt{59c304e}; color stays tessellated \\
C2 & Gate shipped, (a) declined & \texttt{417077b}; undisplaced prepass unsound \\
C3 & Shipped & \texttt{1349387} \\
H4 & Shipped & \texttt{38ae51e} \\
H5 & Shipped (+water) & \texttt{caaec6e} \\
H6 & Shipped (exact) & \texttt{2e566c8} \\
H7 & Partial (interpolant+twins) & \texttt{27c021d}; stride stays 64 \\
H8 & Shipped (redesigned) & \texttt{adfe00f}; heat moved to TES \\
M9 & Shipped (extended) & \texttt{79d7ca1} \\
M10 & Shipped & \texttt{7b0077a} \\
M11 & Shipped (better) & \texttt{a87406e}; fills, not buffers \\
M12 & Partial (exact micro) & \texttt{5456b9b}; chunk constants declined \\
L13--L15 & Shipped & \texttt{f0c2896} \\
\bottomrule
\end{tabular}
\end{table}

``Partial'' marks items whose headline proposal failed exactness review
(C1 color blend, C2 prepass depth, H7 stride, M12 chunk constants) and
shipped in sound subset form instead. ``Declined'' marks the unsound halves,
with proofs in the section notes. Nothing was implemented that the audit
could not verify statically; runtime confirmations (screenshots, timestamps,
VRAM) are recorded per item for a machine with a GPU.
