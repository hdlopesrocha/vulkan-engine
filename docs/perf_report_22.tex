...
\\ \textbf{Implemented as:} (1) shipped --- GPU compute bake
(\texttt{vegetation\_bake.comp}, 1.4k words) of the height scale into a
float-per-instance concatenated aux buffer (+32\,MB worst case), evaluated
synchronously inside \texttt{consolidateChunks} so aux and instances stay in
lockstep; all four VS variants and all six pipelines read it (new binding 2,
\texttt{ATTR\_VEG\_AUX}). Same device, same \texttt{perlin2d.glsl} source,
same operation order: identical up to FMA-contraction ulps on a smooth
signal with no cliffs (invisible by construction). (2) declined pending A/B:
outer-cascade texels (10--80\,cm) make 6-plane detail plausibly sub-texel
(Nyquist), but silhouette lightening needs eyes; the cull/VBO/VS-variant
machinery is mapped in the section above for whoever runs that A/B. True
once-per-frame expansion (cross-pass vertex reuse) needs transform feedback
or mesh shaders --- out of scope.
\\ \textbf{Postscript (missed consumer, fixed):} \texttt{ImpostorCapture}
loads \texttt{vegetation.vert} with its own 5-attribute pipeline, so the new
input broke its creation (VUID-07904 at launch). It draws a canonical single
instance with no aux buffer bound, so it now builds \texttt{vegetation.vert}
with \texttt{VEG\_CAPTURE=1} (height evaluated locally, exactly as before
the bake; disassembly confirms no location-7 input). Lesson: new shader
inputs require auditing every loader of the module, not just the owning
renderer.
\end{tcolorbox}
...
\\ \textbf{Implemented as:} (1) shipped for SDF/bbox (the actual gaps;
brush-liquid was already gated): steady-state-disabled tasks skip
submit/signal entirely (warmup + toggle-transition runs preserve the
clear+read-only target invariant; composite waits only on registered
timelines, so no DAG change); (3) shipped --- cumulative counter next to
\texttt{m\_queueSubmitted} plus a StatsOverlay per-frame readout. (2)
declined: merging the 3 cascade CBs saves 2 submits but serializes
parallel recording on worker cores, and batching them in one submit needs
surgery in the shared submit helper (fences, throttling, abort paths) ---
the record-vs-submit tradeoff is unmeasurable without executing. Corrected
count: 12 MyApp + 5 shadow $=$ 17 submits/frame, not $\sim$18.
\end{tcolorbox}
...
validation-clean.
\\ \textbf{Implemented, narrowed:} height scale only (the $\sim$100-flop
head of the per-vertex bill; density hash at $\sim$12 flops stays
in-shader, trigonometry stays for lack of an A/B-able fidelity story).
All four VS variants disassemble clean of \texttt{vegetationHeightScale}
with the location-7 input live.
\end{tcolorbox}
...
C2 & Veg height bake + aux buffer (shipped) & $-100$ flops/vtx $\times$ 24 $\times$ passes \\
