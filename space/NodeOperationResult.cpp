...
NodeOperationResult::NodeOperationResult(
    OctreeNode * node_, 
    SpaceType shapeType_, 
    const float * shapeSDF_, 
    SpaceType resultType_, 
    const float * resultSDF_, 
    int brushIndex_, 
    uint8_t selectedLod_, 
    uint8_t selectedChunkLod_
): 
    node(node_), 
    shapeType(shapeType_), 
    resultType(resultType_), 
    brushIndex(brushIndex_),
    brushHsv(0.0f, 0.5f, 0.5f),
    selectedLod(selectedLod_), 
    selectedChunkLod(selectedChunkLod_)
{
    if (resultSDF_) 
