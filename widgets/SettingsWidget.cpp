...
void SettingsWidget::render() {
    ...
    if (!wg.visible()) return;
        ...
        ImGuiHelpers::SetTooltipIfHovered("Render leaf-node cube faces colored by SDF sign; frustum-culled on the GPU via indirect.comp (only visible cubes are drawn)");
